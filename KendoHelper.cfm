...
public string function GetGrid( 
	...
	//,numeric height = 400
	,string pKey = ""
	,string component = "services/_Tron.cfc"
	...
	CreateMethod = "JsonCreate";

	ReadMethod = "JsonGet";

	UpdateMethod = "JsonSet";

	DeleteMethod = "JsonDelete";
	...
	try {
		...
		if ( arguments.component == "services/_Tron.cfc" ) {

			generatedOutput = generatedOutput &  '				url: serviceURL + "' &  ReadMethod &  '&model=' & arguments.model & '",' &  chr(10) & chr(10);
		}
		else {
		...
		generatedOutput = generatedOutput &  '				id: "' & Ucase(primaryKey) & '",' & chr(10)  & chr(10);
		...
		for (col = 1; col LTE qModel.RecordCount; col++) {
			...
			if (qModel["IS_PRIMARYKEY"][col]) {
				...
				generatedOutput = generatedOutput & '					' & Ucase(qModel["COLUMN_NAME"][col]) & ': { editable: false, nullable: true }'  ;
				...
				generatedOutput = generatedOutput & '					' & Ucase(qModel["COLUMN_NAME"][col]) 
		...
		for (col = 1; col LTE qModel.RecordCount; col++) {
			...
			if (NOT qModel["IS_PRIMARYKEY"][col]) {
				...
				generatedOutput = generatedOutput &  '			{ field: "' & Ucase(qModel["COLUMN_NAME"][col]) & '", title: "' & Replace(qModel["COLUMN_NAME"][col], "_", " ", "All") & '"}' ;
